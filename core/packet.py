...
class WGPacketReader(object):
    ...
    def position(self, instance=None):
        self.pos += 5
        x,y,z = unpack("!IIB", self.data[self.pos - 5:self.pos])
        return Position(x, y, z, instance)

    def stackPosition(self, instance=None):
        self.pos += 6
        x,y,z, stackPos = unpack("!IIBH", self.data[self.pos - 6:self.pos])
        return StackPosition(x, y, z, stackPos, instance)
...
class WGPacket(object):
    ...
    def send(self, stream):
        if not stream or not len(self.data): return
        data = ''.join(self.data)
        stream.transport.write(data)
        self.data = None
