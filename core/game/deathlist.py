...
class DeathEntry(object):
    def __init__(self, killerId, victimId, unjustified, revenged=0, time=None, war_id=0, deathId=0):
        if not time:
            time = time.time()
        self.id = deathId
    ...
    def saveQuery(self):
        return "(%d, %d, %d, %d, %d. %d)" % (self.id, self.killerId, self.victimId, self.unjustified, self.time, self.revenged, self.warId)
...
def loadDeathList(playerId):
    ...
    for entry in query:
        ...
        entry = DeathEntry(entry[1], entry[2], entry[3], entry[5], entry[4], entry[6], entry[0])
        
        try:
            byVictim[entry[2]].append(entry)
        except:
            byVictim[entry[2]] = [entry]
            
        try:
            byKiller[entry[1]].append(entry)
        except:
            byKiller[entry[1]] = [entry]
