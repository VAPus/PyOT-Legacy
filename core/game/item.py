...
class Item(object):
    ...
    def description(self, player=None, position=None):
        ...
        extra = ""
        if player and (not position or position.x == 0xFFFF or player.inRange(position, 2, 2)): # If position ain't set/known, we're usually in a trade situation and we should show it.
            if self.weight:
                if self.count:
