...
# Unlike Tibia packets, this follows standard network endian, big endian.
# Some other changes:
# * Position is uint32, uint32, uint8
# * ground is z: 127.
# * Stackpos is 16bit.
# * Camera may (legally) be uncentered from the player
# * Effects are uint16.
# * We send a maximum of viewport floors above ground.
# * There is no "maxKnownCreature", or "maxOutfits", or "maxMounts"
...
class WebPacket(WGPacket):
    ...
    def position(self, position):
        self.raw(pack("!IIB", position.x, position.y, position.z))

    # Magic Effect
    def magicEffect(self, pos, type):
        self.raw(pack("!BIIBH", 0x83, pos.x, pos.y, pos.z, type))
