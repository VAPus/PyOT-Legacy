dwarf = genMonster("Dwarf", (69, 6007), "a dwarf")
dwarf.setHealth(90, healthmax=90)
dwarf.bloodType("blood")
dwarf.setDefense(armor=8, fire=0.95, earth=0.9, energy=1, ice=1, holy=1, death=1.05, physical=1, drown=1)
dwarf.setExperience(45)
dwarf.setSpeed(170)
dwarf.setBehavior(summonable=320, hostile=1, illusionable=1, convinceable=320, pushable=0, pushItems=0, pushCreatures=0, targetDistance=1, runOnHealth=0)
dwarf.walkAround(energy=0, fire=0, poison=0)
dwarf.setImmunity(paralyze=1, invisible=1, lifedrain=1, drunk=1)
dwarf.voices("Hail Durin!")
dwarf.regMelee(30)
dwarf.loot( ("hatchet", 24.5), ("leather legs", 10.75), ("white mushroom", 49.0), (3031, 100, 8), ("studded armor", 6.75), ("axe", 16.25), ("letter", 7.5), ("pick", 10.0), ("copper shield", 10.25), ("iron ore", 0.0025, 3), ("dwarven ring", 0.25) )