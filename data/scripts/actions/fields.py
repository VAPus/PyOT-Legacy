def _verify(creature, thing, position):
    ...
    if not creature.hasCondition(getattr(game.enum, 'CONDITION_%s' % (thing.field.upper()))):
        ...
        if thing.fieldStart:
            creature.condition(PercentCondition(getattr(game.enum, 'CONDITION_%s' % (thing.field.upper())), startdmg=thing.fieldStart, percent=0.933)) # Magic constant. I dunno, seems ok for fields hehe.
        else:
...
def callback(creature, thing, position, **k):
    if thing.fieldDamage:
        every = (thing.fieldTicks or 2000)/1000
        if thing.fieldStart:
            creature.condition(PercentCondition(getattr(game.enum, 'CONDITION_%s' % (thing.field.upper())), startdmg=thing.fieldStart, percent=0.933)) # Magick constant. Seems ok for poison fields.
        else:
            creature.condition(Condition(getattr(game.enum, 'CONDITION_%s' % (thing.field.upper())), length=every * (thing.fieldCount or 1), every=every, damage=thing.fieldDamage))
